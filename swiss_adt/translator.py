...
DEFAULT_LANGUAGE_CODES = {"DE": "German", "FR": "French", "IT": "Italian", "EN": "English"}

class Translator:
    def __init__(self, api_key, model="gpt-4o", request_per_second=2, language_code=DEFAULT_LANGUAGE_CODES):
    ...
    @backoff.on_exception(backoff.expo, (requests.exceptions.RequestException, ServerError), max_time=60)
    def translate_segment(self, text, images, source_language, target_language):
        ...
        if source_language not in self.language_code or target_language not in self.language_code:
            raise ValueError("Invalid language code")
        source_language = self.language_code[source_language]
        target_language = self.language_code[target_language]

        text = f"Translate the following audio description for the frames of this video from {source_language} to" \
